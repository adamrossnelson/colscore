...
// Feb/2022:  Resolved todo items related to capture staements.
//            Revised to reference CollegeScorecard_Raw_Data_02072022.zip
// Feb/2022:  Routine updates. Now includes data athrough the 
//            MERGED2018_19_PP data file.
...
// Specify file and location.
local cur_file = "CollegeScorecard_Raw_Data_02072022.zip"
local cur_addy = "https://ed-public-download.app.cloud.gov/downloads/"

// Copy the zip file from the web to our current directory
// Update: Fomer versions referenced data at (which contains a subset of variables)
...
forvalues i = 1996 / 2018 {
	...
	gen int year = `i'
	label variable year "The file year index prefix."
	gen int isYr = year
	label variable isYr "The file year index prefix."
	gen orig_file = "Processing MERGED`i'_`num2'_PP.csv"
	label variable orig_file "The file name from which this record originated."
...
clear
di ""
di "Loading the first year of data (1996)"
di "======================================================================"
...
// Update: CollegeScorecard_Raw_Data.zip contained "ï" char.
//         Variable rename no longer necessary following update to 
//         reference CollegeScorecard_Raw_Data_MMDDYYYY
...
// These files:
//   colscore_val_lab_writes.do
//   colscore_var_lab_writes.do
//   can be refreshed with colscore_build_stata_meta.py

// TODO: Engineer a method to "find" local copy of these files...
// Miscellaneous options for local execution.
// do "`starting_dir'/colscore_val_lab_writes.do"
// do "`starting_dir'/colsgticore_var_lab_writes.do"
// do "~/Documents/Gits/colscore/colscore_val_lab_writes.do"
// do "~/Documents/Gits/colscore/colscore_var_lab_writes.do"

// Write and add variable labels
do https://raw.githubusercontent.com/adamrossnelson/colscore/master/colscore_var_lab_writes.do

// Write and add value labels
